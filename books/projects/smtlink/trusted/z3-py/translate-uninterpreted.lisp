...
(define translate-uninterpreted-arguments ((types symbol-listp)
  ...
  (b* ((types (symbol-list-fix types))
       ...
       (translated-type
        (translate-type first (cdr (assoc-equal first type-alst)))))
...
(define translate-uninterpreted ((funcs symbol-trans-hint-alist-p)
                                 (types symbol-smt-type-alist-p))
  :returns (mv (py-term paragraph-p)
               (smt-property pseudo-term-list-listp))
  :measure (len (symbol-trans-hint-alist-fix funcs))
  (b* ((funcs (symbol-trans-hint-alist-fix funcs))
       (types (symbol-smt-type-alist-fix types))
       ((unless (consp funcs)) (mv nil nil))
       ((cons first rest) funcs)
       ((cons name hints) first)
       ((unless (equal (trans-hint->kind hints) :uninterpreted))
