...
(define translate-one-decl ((decl pseudo-termp)
  ...
  (b* (((unless (is-translatable-decl? decl))
       ...
       ((unless exists?)
        (er hard? 'translate-declarations=>translate-one-decl
            "Unrecognized type ~p0, consider adding it to the hint.~%" type))
...
(define extend-symbol-map-list ((types symbol-smt-datatype-alist-p)
                                (symbol-keeper symbol-keeper-p))
  :returns (new-symbol-keeper symbol-keeper-p)
  :measure (len (symbol-smt-datatype-alist-fix types))
  :hints (("Goal" :in-theory (enable symbol-smt-datatype-alist-fix)))
  (b* ((types (symbol-smt-datatype-alist-fix types))
       (symbol-keeper (symbol-keeper-fix symbol-keeper))
       ((unless (consp types)) symbol-keeper)
       ((cons types-hd types-tl) types)
       ((cons & type) types-hd)
       ((unless (equal (smt-datatype-kind type) :sumtype)) symbol-keeper)
...
(define translate-declarations ((decl-term pseudo-termp)
  ...
  (b* ((decl-term (pseudo-term-fix decl-term))
       ...
       ((trusted-hint h) hint)
       (new-symbol-keeper (extend-symbol-map-list h.user-types symbol-keeper))
       ((symbol-keeper sk) new-symbol-keeper)
       ((mv translated-types user-type-properties)
        (create-datatypes h sk.symbol-map))
       (translated-declaration-list
        (translate-declaration-list (reverse decl-list) h.user-types))
       (translated-syms
