...
(define construct-sum ((rec smt-function-p)
  ...
  (b* ((rec (smt-function-fix rec))
       ...
       (acc-3 (destructor-of-constructors s.constructor s.destructors
                                          formal-vars types acc-2))
       ((unless kind) acc-3)
       (acc-4 (constructor-of-destructor-with-tag rec kind sum return-var
                                                  type types acc-3)))
    (tag-of-constructor kind sum formal-vars types acc-4)))
...
(define construct-sum-list ((rec smt-function-p)
  ...
  (b* ((rec (smt-function-fix rec))
       ...
       ((cons sum-hd sum-tl) sum-lst))
    (construct-sum-list
     rec kind sum-tl return-var type types
     (construct-sum rec kind sum-hd return-var type types acc))))
...
(define sumtype-property ((type smt-datatype-p)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       (acc-2
        (construct-sum-list tp.recognizer tp.kind tp.sums
                            return-var type types acc-1))
       (acc-3
        (constructor-of-destructors tp.recognizer tp.sums
                                    return-var type acc-2))
       ((unless tp.kind) acc-3)
       (acc-4
        (tag-of-all tp.recognizer tp.kind tp.sums return-var types acc-3)))
