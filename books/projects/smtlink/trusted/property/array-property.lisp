...
(define array-select-init ((type smt-datatype-p)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       (the-hint1 (get-hints r.name nil :type-of-init prop-hints))
       (the-hint2 (get-hints r.name nil :equal-of-init prop-hints)))
    `(,(list `(hint-please ',the-hint1) `(,r.name (,f.name)))
      ,(list `(hint-please ',the-hint2)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       ((if (or (equal se.name 'quote) (equal r.name 'quote)))
        (er hard? 'array-property=>array-select-equal
            "Function name should not be 'quote.~%"))
       (the-hint (get-hints r.name nil :type-of-array-select prop-hints)))
    (cons (list `(hint-please ',the-hint)
                `(if (if (,r.name ar) (,key-type k) 'nil)
...
(define array-select-equal ((type smt-datatype-p)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       ((if (or (equal se.name 'quote) (equal st.name 'quote)))
        (er hard? 'array-property=>array-select-equal
            "Function name should not be 'quote.~%"))
       (the-hint (get-hints r.name nil :array-select-equal prop-hints)))
    (cons (list `(hint-please ',the-hint)
                `(if (if (,r.name ar)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       ((if (or (equal se.name 'quote) (equal st.name 'quote)))
        (er hard? 'array-property=>array-select-distinct
            "Function name should not be 'quote.~%"))
       (the-hint (get-hints r.name nil :array-select-distinct prop-hints)))
    (cons (list `(hint-please ',the-hint)
                `(if (if (,r.name ar)
                         (if (,key-type k1)
                             (if (,key-type k2)
...
(define type-of-array-store ((type smt-datatype-p)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       ((if (or (equal st.name 'quote) (equal r.name 'quote)))
        (er hard? 'array-property=>array-select-equal
            "Function name should not be 'quote.~%"))
       (the-hint (get-hints r.name nil :type-of-array-store prop-hints)))
    (cons (list `(hint-please ',the-hint)
                `(if (if (,r.name ar)
                         (if (,key-type k) (,val-type v) 'nil)
                       'nil)
  ...
  (b* ((type (smt-datatype-fix type))
       ...
       ((if (or (equal r.name 'quote) (equal s.name 'quote)
        ...
        (er hard? 'array-property=>array-select-equal-witness
            "Function name should not be 'quote.~%"))
       (the-hint1
        (get-hints r.name nil :array-equal-implies-select-equal prop-hints))
       (the-hint2
        (get-hints r.name nil
                   :select-of-witness-equal-implies-array-equal prop-hints)))
    `(,(list `(hint-please ',the-hint1)
             `(if (if (,r.name a1)
                      (if (,r.name a2)
